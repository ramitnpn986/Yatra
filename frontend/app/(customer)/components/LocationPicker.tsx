...
const LocationMarker = ({ position, setPosition, onSelect, isEditable, }: {
    ...
    useMapEvents({
        click(e) {
            ...
            setPosition(location); onSelect(location);
        },
    });
    return <Marker position={position} />;
};
const LocationPicker = ({ onSelect, currentCoords, isEditable, }: LocationPickerProps) => {
    const [position, setPosition] = useState<Coordinates>(currentCoords);
    useEffect(() => { setPosition(currentCoords); }, [currentCoords]);
    return (
